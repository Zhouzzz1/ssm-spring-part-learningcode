...
-   **3.2.3 SpringIoC容器具体接口和实现类**
    ...
    **SpringIoc容器接口**：&#x20;
    ...
    `ApplicationContext` 是 `BeanFactory` 的子接口。它`ApplicationContext`扩展了以下功能：
...
​    ```
       ...
       - 将一个组件类   声明2个组件信息     默认是单例模式    会实例化2个组件对象
...
5.  **配置文件确定扫描范围**
    ...
    ![image-20240304201517249](image/image-20240304201517249.png)
...
5.  **扩展JSR-250注解@Resource**
    ...
    - JSR-250 @Resource注解
      ...
      ![image-20240305175013767](image/image-20240305175013767.png)
...
![image-20240306083701763](image/image-20240306083701763.png)
...
3.  整合测试注解使用
    ...
    //JavaConfig.class作为配置类，这意味着Spring应用上下文将使用JavaConfig类进行配置
    public class Junit5IntegrationTest {
        
        @Autowired
        /*使用了Spring框架中的@Autowired注解来告诉Spring容器，将一个类型为A的bean（即一个A类的实例）注入到当前类中的字段a中。换句话说，Spring框架会在初始化Junit5IntegrationTest类时，自动创建一个A类的实例，并将它赋值给字段a，这样在测试方法中就可以直接使用这个A类的实例了。*/
        private A a;
...
3.  **动态代理**
    ...
    动态代理技术分类
    -   JDK动态代理：JDK原生的实现方式，需要被代理的目标类必须**实现接口**！他会根据目标类的接口动态生成一个代理对象！代理对象和目标对象有相同的接口！（拜把子）(有接口就用jdk)
    -   cglib：通过继承被代理的目标类实现代理，所以不需要目标类实现接口！（认干爹）(没接口用cglib)
        JDK动态代理技术实现（了解）
        ...
        public Object getProxy(){
            ...
            InvocationHandler invocationHandler = new InvocationHandler() {
                ...
                public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
                    /**
                    invoke 调用代理方法都会执行此方法 非核心业务+目标调用
                     * proxy：代理对象
                     * method：目标方法 代理对象需要实现的方法，即其中需要重写的方法
                     * args：执行方法的参数 method所对应方法的参数
                     */
                    Object result = null;
                    try {
    		...
    		//jdk生成代理对象,参数一：类加载器 参数二：目标类的接口  参数三：具体要进行的代理动作[非核心动作 调用目标方法]
            return Proxy.newProxyInstance(classLoader, interfaces, invocationHandler);
        }
    }
    ```
    ![image-20240307203936572](image/image-20240307203936572.png)
    
    ![image-20240307204029975](image/image-20240307204029975.png)
...
2.  **AOP思想主要的应用场景(就是来解决非核心业务代码冗余问题)**
...
4. 声明切面类
   ...
   */
   ...
   public class LogAspect {
       ...
       //execution可以这么写execution(* com.zhouzzz.service.impl.*.*(..))
       //第一个* 不考虑方法的返回值和访问修饰符 后面的第一个.*是指service包下的所有类,
       //第二个.*上一个类的哪一个方法 (..)方法的插入是什么，代表方法有没有参数都行
...
![image-20240309220332346](image/二、SpringFramework实战指南.md)
...
public class LogAdvice {
    @Before("execution(* com.zhouzzz.service.impl.*.*(..))")
    public void start(){

        System.out.println("方法开始了...");
    }
    @After("execution(* com.zhouzzz.service.impl.*.*(..))")
    public void after()
    {
        System.out.println("方法结束了...");
    }
    @AfterThrowing("execution(* com.zhouzzz.service.impl.*.*(..))")
    public void error()
    {
...
public class Myadvice {
    ...
    @AfterReturning(value = "execution(* com..impl.*.*(..))",returning = "result")//result接返回结果的
...
CalculatorPureImpl.java

```java
package com.zhouzzz.service.impl;
...
/**
 * aop - 只针对ioc容器的对象 - 创建代理对象 -> 代理对象存储到ioc容器中
 */
@Component
public class CalculatorPureImpl implements Calculator {
...
2.  **切点表达式语法**
    ...
    -   第四位：指定包的地址
        ```java
         ...
         找所有impl包 com..impl 不能写..impl 可以写成*..impl
         找到任何包下: *..
        ```
    -   第五位：指定类名称
        ```java
...
@Aspect
public class LogAdvice {
    @Pointcut("execution(* com.zhouzzz.service.impl.*.*(..))")
    public void pc(){}
...
新建一个存储切点的类MyPointCut.java

```java
package com.zhouzzz.pointcut;
...
事务 是一组操作的集合，它是一个不可分割的工作单位，事务会把所有的操作作为一
个整体一起向系统提交或撤销操作请求，即这些操作要么同时成功，要么同时失败。
如果不进行事务控制，可能导致在删除部门信息成功时，该部门下的员工却没被删除，
为了解决上述的问题，就需要通过数据的事务来完成，我们只需要在业务逻辑执行之
前开启事务，执行完毕后提交事务。如果执行过程中报错，则回滚事务，把数据恢复
到事务开始之前的状态。
默认 MySQL 的事务是自动提交的，也就是说，当执行一条 DML 语句时， MySQL 会
立即隐式的提交事务。

回滚是指将该事务已经完成对数据库的更新操作撤销

 @Transactional类和方法都可以加，如果同时加，那就以方法为准
...
2. Spring声明式事务对应事务管理器接口
   ...
   TransactionManger是spring5.2版本才有的
...
​			spring 声明式事务给我们提供了对各管理器实现

​				需要哪种。就加入到ioc容器即可！

​	mybatis jdbc Tempata ->DataSourceTM
...
1.  默认情况
    ...
    public class StudentService {
        ...
        public void changeInfo() throws FileNotFoundException {
            studentDao.updateAgeById(100,1);
            //主动抛出一个检查异常,测试! 发现不会回滚,数据发生了更改，因为不在rollbackFor的默认范围内! 
...
1.  事务隔离级别
    ...
    1.  读未提交（Read Uncommitted）：事务可以读取未被提交的数据，容易产生脏读(一个事务读取了另外一个事务未提交的数据)、不可重复读(一个事务读取了另外一个事务提交修改的数据)和幻读(一个事务读取了另外一个事务提交的插入的数据)等问题。实现简单但不太安全，一般不用。
    2.  读已提交（Read Committed）：事务只能读取已经提交的数据，可以避免脏读问题，但可能引发不可重复读和幻读。Oracle的默认隔离级别
    3.  可重复读（Repeatable Read）：在一个事务中，相同的查询将返回相同的结果集，不管其他事务对数据做了什么修改。可以避免脏读和不可重复读，但仍有幻读的问题。Mysql的默认隔离级别
...
2. propagation属性
   ...
   | 名称                 | 含义                                                         |
   | -------------------- | ------------------------------------------------------------ |
   | REQUIRED &#xA;默认值 | 如果父方法有事务，就加入，如果没有就新建自己独立！推荐使用默认值 |
   | REQUIRES\_NEW        | 不管父方法是否有事务，我都新建事务，都是独立的！ 2个事务或者三个事务 |
3. 测试
4. ![image-20240311201508965](image/image-20240311201508965.png)
   ...
   1.  声明两个业务方法
       ...
       public class StudentService {
           ...
           public void changeName(){
               ...
               int i = 1/0;//报错，当前事务会回滚，但用的是默认传播行为，与changeAge()虽然是不同方法，但是是同一个事务，所以会导致changeAge()方法也会回滚(数据库中changeAge()没改变数据)，证明Propagation.REQUIRED生效了
           }
...
![image-20240311211327707](image/image-20240311211327707.png)
